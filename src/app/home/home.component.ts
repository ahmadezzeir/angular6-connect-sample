...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subsGetManager = this.homeService.getMyManager().subscribe(manager => {
      this.manager = manager;
      console.log(this.manager);
    });
    this.subsGetPhoto = this.homeService.getPhoto().subscribe(photo => {
  ...
  onSendMail() {
    this.message = {
      subject:
        "Welcome to Microsoft Graph development with Angular 4 and the Microsoft Graph Connect sample",
      toRecipients: [
        {
          emailAddress: {
  ...
  //   this.homeService.getPhoto().subscribe(
  //     data => {
